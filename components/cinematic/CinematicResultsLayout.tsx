...
import { CinematicTrackTable } from "./CinematicTrackTable";
...
export function CinematicResultsLayout({ initialQuery, tracks, totalCount }: Props) {
    ...
    <>
      ...
      <div style={{ position: "sticky", top: 56, zIndex: 40, backgroundColor: "var(--color-surface)", borderBottom: "1px solid var(--color-outline-variant)", padding: "12px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 24 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, overflowX: "auto", minWidth: 0 }}>
            {FILTERS.map((filter) => (
              <button key={filter} style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 14px", borderRadius: 9999, backgroundColor: "var(--color-surface-container-highest)", border: "none", fontFamily: "var(--font-body)", fontSize: 12, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--color-on-surface-variant)", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                {filter}
                <span className="material-symbols-outlined" style={{ fontSize: 16 }}>expand_more</span>
              </button>
            ))}
            <button style={{ fontFamily: "var(--font-body)", fontSize: 12, fontWeight: 500, color: "var(--color-on-surface-variant)", background: "none", border: "none", cursor: "pointer", whiteSpace: "nowrap", marginLeft: 8 }}>
              Clear all
            </button>
          </div>
          {/* Sort */}
      ...
      <main style={{ backgroundColor: "var(--color-surface)", paddingBottom: 128, minHeight: "100vh" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "16px 24px 0" }}>
          <CinematicTrackTable tracks={tracks} totalCount={totalCount} />
        </div>
      </main>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 50, height: 80, backgroundColor: "var(--color-surface-container-lowest)", borderTop: "1px solid var(--color-outline-variant)", boxShadow: "var(--shadow-player)", padding: "0 24px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 14, fontWeight: 700, color: "var(--color-on-surface)" }}>{audio.state.title || "No track"}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            ...
            <button style={{ width: 40, height: 40, borderRadius: 9999, backgroundColor: "var(--color-secondary)", color: "var(--color-surface)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><span className="material-symbols-outlined" style={{ fontSize: 24, fontVariationSettings: "'FILL' 1" }}>play_arrow</span></button>
          ...
          <div style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "0 16px" }}>
            ...
            <div style={{ flex: 1, height: 2, backgroundColor: "var(--color-surface-container-highest)", borderRadius: 9999, overflow: "hidden" }}><div style={{ height: "100%", width: "0%", backgroundColor: "var(--color-secondary)" }} /></div>
            <span style={{ fontFamily: "var(--font-body)", fontSize: 10, color: "var(--color-on-surface-variant)", width: 28 }}>0:00</span>
          </div>
        </div>
        {/* Volume */}
